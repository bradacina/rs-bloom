...
fn hashlittle2(key: &[u8], pc: u32, pb: u32) -> (u32, u32)
{
  ...
  if is_little_endian && ((alignment & 0x3) == 0) {
    ...
    12 => {c= c.wrapping_add(k[2]); b = b.wrapping_add(k[1]); a= a.wrapping_add(k[0]);}
    11 => {c= c.wrapping_add(k[2]&0xffffff); b=b.wrapping_add(k[1]); a=a.wrapping_add(k[0]);}
    10 => {c=c.wrapping_add(k[2]&0xffff); b=b.wrapping_add(k[1]); a=a.wrapping_add(k[0]);}
    9 => {c=c.wrapping_add(k[2]&0xff); b=b.wrapping_add(k[1]); a=a.wrapping_add(k[0]);}
    8 => {b=b.wrapping_add(k[1]); a=a.wrapping_add(k[0]);}
    7 => {b=b.wrapping_add(k[1]&0xffffff); a=a.wrapping_add(k[0]);}
    6 => {b=b.wrapping_add(k[1]&0xffff); a=a.wrapping_add(k[0]);}
    5 => {b=b.wrapping_add(k[1]&0xff); a=a.wrapping_add(k[0]);}
    ...
    while length > 12
    {
      a = a.wrapping_add(k[0] as u32).wrapping_add((k[1] as u32)<<16);
      b = b.wrapping_add(k[2] as u32).wrapping_add((k[3] as u32)<<16);
      c = c.wrapping_add(k[4] as u32).wrapping_add((k[5] as u32)<<16);
      mix(&mut a,&mut b,&mut c);
      length -= 12;
      k = &k[6..];
    }
    ...
    let k8: &[u8] = unsafe { std::mem::transmute(k)};
    match length {
    12 => {c=c.wrapping_add(k[4] as u32).wrapping_add((k[5] as u32)<<16);
             b=b.wrapping_add(k[2] as u32).wrapping_add((k[3] as u32)<<16);
             a=a.wrapping_add(k[0] as u32).wrapping_add((k[1] as u32)<<16);
    }
    11=> {
    ...
    10=> { c=c.wrapping_add(k[4] as u32);
             b=b.wrapping_add(k[2] as u32).wrapping_add((k[3] as u32)<<16);
             a=a.wrapping_add(k[0] as u32).wrapping_add((k[1] as u32)<<16);
    }
    9 => {
...
#[cfg(test)]
mod tests {
  use super::{hashlittle, hashlittle2};
  ...
  #[test]
  fn test_hashlittle2() {
    let (b,c) = hashlittle2(b"", 0,0);
    assert_eq!(c, 0xdeadbeef);
    assert_eq!(b, 0xdeadbeef);

    let (b,c) = hashlittle2(b"", 0, 0xdeadbeef);
    assert_eq!(c, 0xbd5b7dde);
    assert_eq!(b, 0xdeadbeef);

    let (b,c) = hashlittle2(b"", 0xdeadbeef, 0xdeadbeef);
    assert_eq!(c, 0x9c093ccd);
    assert_eq!(b, 0xbd5b7dde);

    let (b,c) = hashlittle2(b"Four score and seven years ago", 0, 0);
    assert_eq!(c, 0x17770551);
    assert_eq!(b, 0xce7226e6);

    let (b,c) = hashlittle2(b"Four score and seven years ago", 0, 1);
    assert_eq!(c, 0xe3607cae);
    assert_eq!(b, 0xbd371de4);

    let (b,c) = hashlittle2(b"Four score and seven years ago", 1, 0);
    assert_eq!(c, 0xcd628161);
